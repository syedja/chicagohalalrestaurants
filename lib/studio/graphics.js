...
/**
 * Draw a promo graphic onto the given canvas.
 * opts: { template, format, eyebrow, headline, subline, restaurantName,
 *         contact, cta, logoImg (HTMLImageElement|null) }
 */
export function drawGraphic(canvas, opts) {
  ...
  const s = f.w / 1080; // scale factor relative to base design
  ...
  // Vertical layout cursor. Story format centers the block lower.
  let y = isStory ? f.h * 0.2 : isWide ? 96 * s : 190 * s;

  // Logo (optional, drawn above eyebrow, circle-clipped)
  if (opts.logoImg) {
  ...
  // CTA pill sits above footer text when present
  if (opts.cta) {
